...
namespace Sublime {
	...
	class Command extends \WP_Parser\Command {
		...
		protected function _get_phpdoc_data( $path, $format = 'json' ) {
			 ...
			 * Default: false
			 */
			$delete_output_cache_file = apply_filters( 'sublime_delete_phpdoc_output_cache', false );

			/**
			 * Force delete last cache of phpdoc
			 *
			 * Only run if $delete_output_cache_file = false
			 *
			...
			if ( ! $delete_output_cache_file ) {
				...
				if ( false !== strpos( $wp_parser_root_import_dir, $directory_to_compare ) ) {
					...
					$delete_output_cache_file      = version_compare( $current_wp_version, $wp_parser_imported_wp_version, '>' );
				}
			}
