...
class Functions extends Exporter {
		...
		if ( 1 === count( $data ) && ! isset( $data[0]['childrens'] ) ) {
			if ( false === strpos( $data[0]['name'], 'deprecated' ) ) {
				if ( isset( $data[0]['default_value'] ) ) {
					$arguments = sprintf( '${1: ${2:%s} }', $this->parse_argument_name( $data[0]['default_value'], 3 ) );
				} else {
	...
	public function argument( $arguments, $data, $index ) {
		...
		if ( false === stripos( $data['name'], 'deprecated' ) ) {
			...
			// Fix index
			if ( $name === $this->parse_argument_name( $name, $index ) ) {
				$index = ( $index - 1 );
			} else {
				$index = ( count( preg_split('/\$\{[0-9]+:/', $arguments ) ) - 1 );
			}

		} else {
			$arguments .= sprintf( '%s%s', ( false !== strpos( $arguments, '${' ) ? ', ' : '' ), $name );
		}
	...
	public function parse_argument_name( $name, $index ) {
		if ( 'array()' === $name ) {
			$name = sprintf( 'array( ${%s:\'${%s:}\'} )', $index, ++$index );
		} elseif ( in_array( $name[0], array( "'", '"' ) ) ) {
			$name = sprintf( '\'${%s:%s}\'', $index, str_replace( array( "'", '"' ), '', $name ) );
		}
